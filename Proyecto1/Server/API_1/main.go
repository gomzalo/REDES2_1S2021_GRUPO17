...
func CrearReporteHandler(w http.ResponseWriter, request *http.Request) {
	...
	c := datapb.NewDataServiceClient(cc)

	res, err := c.SaveReporte(context.Background(), &data)
	...
	fmt.Println("Respuesta: ", res.Description)
	return
	// var reportes []Reporte
...
func CrearAsistenciaHandler(w http.ResponseWriter, request *http.Request) {
	enableCors(&w)
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	// (w).Header().Set("Access-Control-Allow-Origin", "*")
	// (w).Header().Set("Access-Control-Allow-Methods", "GET, OPTIONS")
	...
	res, err := c.SaveAsistencia(context.Background(), &data)
...
func main() {
	...
	router.HandleFunc("/asistencia", CrearAsistenciaHandler).Methods("POST")
